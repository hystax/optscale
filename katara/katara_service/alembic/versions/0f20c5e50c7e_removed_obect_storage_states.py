...
new_states = sa.Enum(
    'created', 'started', 'getting_scopes', 'got_scopes',
    'getting_recipients', 'got_recipients', 'generating_data',
    'generated_data', 'putting_to_herald', 'completed', 'error')
task_table = table(
        'task',
...
def upgrade():
    ...
    try:
        update_task_stmt = update(task_table).values(
            state='generating_data'
        ).where(
            task_table.c.state.in_([
                'generated_data', 'putting_to_object_storage']))
        update_task_stmt2 = update(task_table).values(
