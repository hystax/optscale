...
from katara_service.models.models import Recipient, RolePurpose
from katara_service.utils import (check_kwargs_empty,
...
class RecipientController(BaseController):
    ...
    def _validate(self, recipient, is_new=True, **kwargs):
        query = self.session.query(exists().where(
            and_(*(recipient.get_uniqueness_filter(is_new)))))
