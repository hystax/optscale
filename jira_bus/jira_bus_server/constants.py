...
class BackendUrls:
    ...
    issue_attachment_collection = f'{url_prefix}/shareable_resource/(?P<resource_id>[^/]+)/issue_attachment'
    issue_attachment_item = f'{url_prefix}/issue_attachment/(?P<attachment_id>[^/]+)'
    shareable_book_collection = f'{url_prefix}/shareable_resource/(?P<resource_id>[^/]+)/shareable_book'
