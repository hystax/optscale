...
from jira_bus.jira_bus_server.atlassian_client.client import AtlassianClient
...
from jira_bus.jira_bus_server.models.models import (
    AppInstallation,
    UserAssignment,
    OrganizationAssignment,
)
from jira_bus.jira_bus_server.utils import tp_executor
...
class BaseController:
    ...
    def _get_rest_client(self, token=None, secret=None) -> RestClient:
        return RestClient(url=self._config.restapi_url(), token=token, secret=secret)

    def _get_auth_client(self, token=None, secret=None) -> AuthClient:
        return AuthClient(url=self._config.auth_url(), token=token, secret=secret)

    @staticmethod
    def _get_atlassian_client(app_installation):
        base_url = json.loads(app_installation.extra_payload)["baseUrl"]
        return AtlassianClient(APP_KEY, base_url, app_installation.shared_secret)
    ...
    def _get_app_installation_by_client_key(
        self, client_key, raise_not_found=True
    ) -> AppInstallation:
        app_installation = self._get_model(AppInstallation, client_key=client_key)
    ...
    def _get_user_assignment_by_account_id(
        self, account_id, raise_not_found=True
    ) -> UserAssignment:
        user_assignment = self._get_model(UserAssignment, jira_account_id=account_id)
    ...
    def _get_user_assignment_by_secret(
        self, secret, raise_invalid=True
    ) -> UserAssignment:
        user_assignment = (
            self.session.query(UserAssignment)
            .filter(
    ...
    def _get_org_assignment_by_org_id(
        self, organization_id, raise_not_found=True
    ) -> OrganizationAssignment:
        org_assignment = (
            self.session.query(OrganizationAssignment)
            .filter(
        ...
        if not org_assignment and raise_not_found:
            raise NotFoundException(Err.OJ0008, ["organization_id", organization_id])
    ...
    def get_auth_token(self, account_id):
        ...
        return self._get_auth_token_by_auth_user_id(user_assignment.auth_user_id)
    ...
    def _is_module_in_trace(module_names):
        ...
        for frame_info in frames:
            ...
            for module_name in module_names:
                if cur_module_name == module_name or cur_module_name.startswith(
                    module_name + "."
                ):
                    return True
        return False
    ...
    async def get_future(self, meth_name, *args, **kwargs):
        ...
        try:
            ...
            if self._is_module_in_trace(handled_modules):
                ...
                raise OptHTTPError(
                    exc.response.status_code, Err.OJ0020, [error_dict.get("reason")]
                )
            raise
