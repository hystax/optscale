...
class TestOrganizationApi(TestApiBase):
    ...
    def test_organization_list(self):
        client = self.get_client(
            include_client_key=True, include_account_id=True)
        organization_list_response = self.get_response_mock(
            'organization_list')
        ...
        edit_partner_ids = {
            x[1] for x in actions_resources_edit_partner_response[
                'EDIT_PARTNER']}
        self.rest_cl_mock.organization_list.return_value = (
            200, organization_list_response)
        self.auth_cl_mock.action_resources_get.return_value = (
            200, actions_resources_edit_partner_response)
        code, resp = client.organization_list()
        self.assertEqual(code, 200)
        for org in resp['organizations']:
            self.assertTrue(
                org['id'] in mocked_organizations_by_id)
            self.assertEqual(
                org['name'], mocked_organizations_by_id[org['id']]['name'])
            self.assertEqual(
                org['is_manager'], org['id'] in edit_partner_ids)
