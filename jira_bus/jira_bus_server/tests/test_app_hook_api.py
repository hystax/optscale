...
class TestAppHookApi(TestApiBase):
    ...
    def test_issue_hooks(self):
        ...
        patch('jira_bus_server.controllers.app_hook.AppHookController.'
              '_parse_issue_payload',
              return_value=issue_details).start()

        issue_attachment_list_response = self.get_response_mock(
            'issue_attachment_list')
        self.rest_cl_mock.jira_issue_attachment_list.return_value = (
            200, issue_attachment_list_response)
