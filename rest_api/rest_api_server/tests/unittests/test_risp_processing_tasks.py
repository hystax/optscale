...
class TestRispProcessingTaskApi(TestApiBase):
    ...
    def test_create(self):
        code, resp = self.client.risp_processing_task_create(
    ...
    def test_empty_list(self):
        code, resp = self.client.risp_processing_task_list(
            self.cloud_acc1['id'])
        self.assertEqual(code, 200)
        self.assertEqual(resp['risp_processing_tasks'], [])

    def test_list(self):
        params = self.valid_body.copy()
        code, task1 = self.client.risp_processing_task_create(
            self.cloud_acc1['id'], params)
        self.assertEqual(code, 201)
        params['end_date'] += 1
        code, task1_1 = self.client.risp_processing_task_create(
            self.cloud_acc1['id'], params)
        self.assertEqual(code, 201)
        code, task2 = self.client.risp_processing_task_create(
            self.cloud_acc2['id'], params)
        self.assertEqual(code, 201)

        code, resp = self.client.risp_processing_task_list(
            self.cloud_acc1['id'])
        self.assertEqual(code, 200)
        tasks = resp['risp_processing_tasks']
        ...
        code, _ = self.client.risp_processing_task_list(None)
        self.assertEqual(code, 404)

    def test_delete_not_existing(self):
        code, resp = self.client.risp_processing_task_delete('fake')
        self.assertEqual(code, 404)
        self.verify_error_code(resp, 'OE0002')

    def test_delete(self):
        code, task = self.client.risp_processing_task_create(
            self.cloud_acc1['id'], self.valid_body)
        self.assertEqual(code, 201)
        code, _ = self.client.risp_processing_task_delete(task['id'])
        self.assertEqual(code, 204)
        code, resp = self.client.risp_processing_task_list(
            self.cloud_acc1['id'])
        self.assertEqual(resp['risp_processing_tasks'], [])
    ...
    def test_delete_invalid(self):
        code, resp = self.client.risp_processing_task_create(
            self.cloud_acc1['id'], self.valid_body)
        deleted_task_id = resp['id']
        code, resp = self.client.risp_processing_task_delete(deleted_task_id)
        self.assertEqual(code, 204)

        for task_id in ['impostor', deleted_task_id]:
            code, resp = self.client.risp_processing_task_delete(task_id)
            self.assertEqual(code, 404)
            self.verify_error_code(resp, 'OE0002')

    def test_403(self):
        self.client.secret = None
        code, _ = self.client.risp_processing_task_create(
            self.cloud_acc1['id'], self.valid_body)
        self.assertEqual(code, 403)
        code, _ = self.client.risp_processing_task_list(
            self.cloud_acc1['id'])
        self.assertEqual(code, 403)
        code, _ = self.client.risp_processing_task_delete('fake')
        self.assertEqual(code, 403)
