...
from rest_api.rest_api_server.models.enums import ConstraintLimitStates, ConstraintTypes
...
class TestObserver(TestApiBase):
    ...
    def test_observe_newly_discovered_resources_clustered(self):
        patch('rest_api.rest_api_server.controllers.limit_hit.'
    ...
    @patch('rest_api.rest_api_server.controllers.limit_hit.LimitHitsController.'
           'send_alerts')
    def test_process_limit_hits(self, p_alerts):
        ...
        patch('rest_api.rest_api_server.controllers.resource_constraint.'
    ...
    def test_send_env_changes_activities_task(self):
        ...
        p_send_msg = patch('rest_api.rest_api_server.controllers.base.'
                           'BaseController.publish_activities_task').start()
        patch(
            'rest_api.rest_api_server.controllers.resource_observer.'
    ...
    def test_policy_constraint_limit_hit(self):
        patch('rest_api.rest_api_server.controllers.limit_hit.'
              'LimitHitsController.send_alerts').start()
    ...
    def test_no_green_hit_on_zero_exp(self):
        patch('rest_api.rest_api_server.controllers.limit_hit.'
              'LimitHitsController.send_alerts').start()
        code, pool = self.client.pool_create(
            self.org_id, {'name': 'pool1'})
        ...
        self.assertEqual(code, 200)
        self.assertEqual(len(hits['limit_hits']), 2)
        self.assertEqual(hits['limit_hits'][1]['state'], 'green')
