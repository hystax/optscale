...
class TestLeaderboardTemplatesApi(TestProfilingBase):
    ...
    def test_create_leaderboard_template(self):
        code, resp = self.client.leaderboard_template_create(
            self.org['id'], self.task1['id'], self.valid_leaderboard_template)
        self.assertEqual(code, 201)
        primary_metric = self.valid_leaderboard_template.pop('primary_metric')
        self.assertEqual(resp['primary_metric']['id'], primary_metric)
        filters = self.valid_leaderboard_template.pop('filters')
        for k, v in filters[0].items():
            self.assertEqual(resp['filters'][0][k], v)
        for k, v in self.valid_leaderboard_template.items():
            self.assertEqual(resp[k], v)

    def test_create_req_params(self):
        for req in ['primary_metric', 'grouping_tags']:
            params = self.valid_leaderboard_template.copy()
            params.pop(req)
            code, resp = self.client.leaderboard_template_create(
    ...
    def test_get_leaderboard_template(self):
        code, resp = self.client.leaderboard_template_get(
            self.org['id'], self.task1['id'])
        self.assertEqual(code, 200)
        self.assertEqual(resp, {})
        code, lb_template = self.client.leaderboard_template_create(
            self.org['id'], self.task1['id'], self.valid_leaderboard_template)
        self.assertEqual(code, 201)
        code, resp = self.client.leaderboard_template_get(
            self.org['id'], self.task1['id'])
        self.assertEqual(code, 200)
        self.assertEqual(resp, lb_template)

    def test_delete_leaderboard_template(self):
        code, lb_template = self.client.leaderboard_template_create(
            self.org['id'], self.task1['id'], self.valid_leaderboard_template)
        self.assertEqual(code, 201)
        code, resp = self.client.leaderboard_template_delete(
            self.org['id'], lb_template['task_id'])
    ...
    def test_update_unexpected(self):
        ...
        self.assertEqual(code, 201)
        for k in ['key', 'id']:
            code, resp = self.client.leaderboard_template_update(
                self.org['id'], lb_template['task_id'],
                {k: 'value'})
            self.assertEqual(code, 400)
            self.verify_error_code(resp, 'OE0212')
    ...
    def test_update(self):
        ...
        code, _ = self.client.leaderboard_template_create(
            self.org['id'], self.task1['id'], self.valid_leaderboard_template)
        self.assertEqual(code, 201)
    ...
    def test_details(self):
        ...
        code, resp = self.client.leaderboard_template_get(
            self.org['id'], self.task1['id'], details=True)
    ...
    def test_dataset_coverage_rules(self):
        ...
        self.valid_leaderboard_template['dataset_coverage_rules'] = rules
        code, lb_template = self.client.leaderboard_template_create(
            self.org['id'], self.task1['id'], self.valid_leaderboard_template)
        self.assertEqual(code, 201)
        self.assertEqual(lb_template['dataset_coverage_rules'], rules)
