...
from rest_api.rest_api_server.handlers.v2.profiling.leaderboard_templates import (
    LeaderboardTemplateBaseHandler)
...
class LeaderboardBase(LeaderboardTemplateBaseHandler):

    @staticmethod
    def get_required_params():
        return LeaderboardTemplateBaseHandler.get_required_params() + [
            'dataset_ids', 'name']
    ...
    async def _update_params(self, body, leaderboard_template_id, token):
        leaderboard_ctrl = LeaderboardTemplateAsyncController(
            self.session(), self._config, self.token)
        leaderboard = await run_task(leaderboard_ctrl.get_by_id,
                                     leaderboard_template_id, token)
...
class LeaderboardsAsyncGenerateHandler(LeaderboardBase):

    async def get(self, organization_id, leaderboard_id, **kwargs):
        """
        ---
        description: |
        ...
        res = await run_task(self.controller.generate, leaderboard_id,
                             token)
        self.write(json.dumps(res, cls=ModelEncoder))


class LeaderboardsAsyncCollectionHandler(LeaderboardBase):

    async def post(self, organization_id, leaderboard_template_id, **url_params):
        """
        ---
        description: |
            Create leaderboard
            ...
            201:
                ...
                schema:
                    ...
                    example:
                        ...
                        leaderboard_template_id: 05c1f12e-588b-4108-8b74-f48590fd23b9
                        name: my lbds
        ...
        res = await run_task(self.controller.create, leaderboard_template_id,
                             token, **data)
        self.set_status(201)
        self.write(json.dumps(res, cls=ModelEncoder))

    async def get(self, organization_id, leaderboard_template_id):
        """
        ---
        description: |
            Get leaderboard list by leaderboard template ID
            Required permission: INFO_ORGANIZATION
        tags: [leaderboards]
        ...
        token = await self._get_profiling_token(organization_id)
        res = await run_task(
            self.controller.list, leaderboard_template_id, token)
        self.write(json.dumps(res, cls=ModelEncoder))


class LeaderboardsAsyncItemHandler(LeaderboardBase):
    ...
    async def get(self, organization_id, leaderboard_id, **kwargs):
        ...
        token = await self._get_profiling_token(organization_id)
        res = await run_task(
            self.controller.get, leaderboard_id, token, details)
        self.write(json.dumps(res, cls=ModelEncoder))

    async def patch(self, organization_id, leaderboard_id, **kwargs):
        """
        ---
        description: |
            Update leaderboard
        ...
        res = await run_task(self.controller.edit, leaderboard_id, token, **data)
        self.write(json.dumps(res, cls=ModelEncoder))

    async def delete(self, organization_id, leaderboard_id, **kwargs):
        """
        ---
        description: |
        ...
        await self.check_permissions(
            'EDIT_PARTNER', 'organization', organization_id)
        token = await self._get_profiling_token(organization_id)
        await run_task(self.controller.delete, leaderboard_id, token)
        self.set_status(204)
