...
from rest_api.rest_api_server.utils import Config, is_match_domain
...
class EmployeeController(BaseController, MongoMixin):
    ...
    def notification_domain_blacklist(self):
        try:
            return self._config.marketing_domains_blacklist()
    ...
    def send_new_employee_email(self, organization_id, organization_name,
        ...
        for domain in self.notification_domain_blacklist:
            if is_match_domain(domain, user_email):
                return
        domain_whitelist = self.notification_domain_whitelist
        if domain_whitelist:
            in_whitelist = any(filter(
                lambda x: is_match_domain(x, user_email), domain_whitelist))
            if not in_whitelist:
                return
