import uuid
from auth.auth_server.models.models import Action, ActionGroup, Type, Role
...
class TestType(TestModelBase):
    ...
    def test_role_action_assign(self):
        ...
        role_name = 'admin'
        type_root = Type(id_=0, name=type_name_root)
        ...
        action_run_cs = Action(name=action_name_run_cs, type_=type_root,
                               action_group=cs_action_group)
        role_admin = Role(name=role_name, type_=type_root, lvl=type_root,
        ...
        self.assertTrue(all(map(
            lambda x: x.name in [action_name_create_customer,
                                 action_name_run_cs],
            role_admin.actions)))
