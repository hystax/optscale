""""manage_invites_action"

Revision ID: 12ce2177c12b
Revises: 2a75057a4cb3
Create Date: 2020-04-05 12:41:16.221800
...
revision = '12ce2177c12b'
down_revision = '2a75057a4cb3'
branch_labels = None
depends_on = None
...
def downgrade():
    ...
    try:
        action = session.query(Action).filter_by(
            name='MANAGE_INVITES').one_or_none()
        role_superadmin = session.query(Role).filter_by(
            name='Super Admin').one_or_none()
        role_optscale_manager = session.query(Role).filter_by(
            purpose=RolePurpose.optscale_manager).one_or_none()
