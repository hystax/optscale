...
Revision ID: a130f8e76eb9
Revises: 57c54e0d387d
Create Date: 2018-04-23 22:41:57.628101
...
revision = 'a130f8e76eb9'
down_revision = '57c54e0d387d'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    try:
        glob_mgmt_group = session.query(ActionGroup).filter_by(
            name='GLOBAL_MANAGEMENT', order=1200).one_or_none()

        root_type = session.query(Type).filter_by(name='root').one_or_none()

        action_manage_clouds = Action(name='MANAGE_CLOUDS', type=root_type,
                                      action_group=glob_mgmt_group,
                                      order=1220)
        session.add(action_manage_clouds)
        ...
        try:
            ...
            role_superadmin.assign_action(action_manage_clouds)
...
def downgrade():
    ...
    session = Session(bind=bind)
    try:
        action_manage_clouds = session.query(Action).filter_by(
            name='MANAGE_CLOUDS').one_or_none()
        role_superadmin = session.query(Role).filter_by(
            name='Super Admin').one_or_none()
        role_superadmin.remove_action(action_manage_clouds)
        session.delete(action_manage_clouds)
        session.commit()
    finally:
        session.close()
    # ### end Alembic commands ###
