...
Revision ID: 48dd5187b11e
Revises: 3bd5237d912e
Create Date: 2020-03-25 10:01:09.118300
...
revision = '48dd5187b11e'
down_revision = 'dee5a8a8b0a9'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    try:
        ...
        action_info_organization = Action(name='INFO_ORGANIZATION',
                                          type=partner_type,
                                          action_group=optscale_ag,
                                          order=1360)
        session.add(action_info_organization)
        role_superadmin = session.query(Role).filter_by(
            name='Super Admin').one_or_none()
        role_superadmin.assign_action(action_info_organization)

        type_root = session.query(Type).filter_by(
            name='root').one_or_none()
        role_manager = Role(name='Manager', type=type_root, lvl=type_root,
                            description='Optscale manager', shared=True,
                            purpose=RolePurpose.optscale_manager)
        role_engineer = Role(name='Engineer', type=type_root, lvl=type_root,
                             description='Optscale engineer', shared=True,
                             purpose=RolePurpose.optscale_engineer)
        role_member = Role(name='Member', type=type_root, lvl=type_root,
        ...
        role_actions_map = {
            role_manager: ['INFO_ORGANIZATION', 'MANAGE_CLOUD_CREDENTIALS',
                           ...
                           'MANAGE_BUDGETS', 'MANAGE_PERMISSIONS'],
            role_engineer: ['INFO_ORGANIZATION', 'MANAGE_OWN_RESOURCES'],
            role_member: ['INFO_ORGANIZATION']
        }
...
def downgrade():
    ...
    try:
        action = session.query(Action).filter_by(
            name='INFO_ORGANIZATION').one_or_none()
        role_superadmin = session.query(Role).filter_by(
            name='Super Admin').one_or_none()
        ...
        type_root = session.query(Type).filter_by(
            name='root').one_or_none()
        role_manager = session.query(Role).filter_by(
            name='Manager', type=type_root).one_or_none()
        role_engineer = session.query(Role).filter_by(
            name='Engineer', type=type_root).one_or_none()
        role_member = session.query(Role).filter_by(
            name='Member', type=type_root).one_or_none()
        role_actions_map = {
            role_manager: ['INFO_ORGANIZATION', 'MANAGE_CLOUD_CREDENTIALS',
                           'MANAGE_RESOURCES', 'MANAGE_OWN_RESOURCES',
                           'MANAGE_BUDGETS', 'MANAGE_PERMISSIONS'],
        ...
        for role, actions in role_actions_map.items():
            role_actions = session.query(RoleAction).filter_by(
                role_id=role.id
            ).all()
            for role_action in role_actions:
                session.delete(role_action)
            session.delete(role)
        session.commit()
    except:
        session.rollback()
