""""added_manage_checklists_action"

Revision ID: ff740b5b3df3
Revises: d0609ecf1e68
Create Date: 2020-09-14 08:48:12.194986
...
revision = 'ff740b5b3df3'
down_revision = 'd0609ecf1e68'
branch_labels = None
depends_on = None
...
def downgrade():
    ...
    try:
        action = session.query(Action).filter_by(
            name='MANAGE_CHECKLISTS').one_or_none()
        role_superadmin = session.query(Role).filter_by(
            name='Super Admin').one_or_none()
        role_optscale_manager = session.query(Role).filter_by(
            purpose=RolePurpose.optscale_manager).one_or_none()
