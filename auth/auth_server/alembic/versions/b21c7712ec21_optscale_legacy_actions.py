""""optscale_legacy_actions"

Revision ID: b21c7712ec21
Revises: 12ce2177c12b
Create Date: 2020-04-16 14:50:09.118300
...
revision = 'b21c7712ec21'
down_revision = '12ce2177c12b'
branch_labels = None
depends_on = None
...
LEGACY_ACTIONS = {
    'optscale_member': [
        'POLL_EVENT', 'INFO_PARTNER', 'INFO_CS', 'INFO_DRPLAN',
        'INFO_CUSTOMER'],
    'optscale_engineer': [
        'MANAGE_FAILBACK', 'ACK_EVENT', 'POLL_EVENT', 'INFO_PARTNER',
        'EDIT_RETENTION', 'EDIT_SCHEDULE', 'EDIT_REPLICATION_STATE',
        'DOWNLOAD_AGENT', 'MANAGE_DEVICE', 'INFO_CS', 'CREATE_CS', 'EDIT_CS',
        'DELETE_CS', 'CREATE_DRPLAN', 'EDIT_DRPLAN', 'DELETE_DRPLAN',
        'INFO_DRPLAN', 'MANAGE_SCRIPTS', 'INFO_CUSTOMER'],
    'optscale_manager': [
        'MANAGE_FAILBACK', 'ACK_EVENT', 'POLL_EVENT', 'INFO_PARTNER',
        'CREATE_PARTNER', 'EDIT_PARTNER', 'DELETE_PARTNER', 'EDIT_RETENTION',
        'EDIT_SCHEDULE', 'EDIT_REPLICATION_STATE',  'DOWNLOAD_AGENT',
        'CREATE_GROUP', 'EDIT_GROUP', 'MANAGE_DEVICE', 'INFO_CS', 'CREATE_CS',
        'EDIT_CS', 'DELETE_CS', 'CREATE_DRPLAN', 'EDIT_DRPLAN',
        'DELETE_DRPLAN', 'INFO_DRPLAN', 'MANAGE_SCRIPTS', 'INFO_CUSTOMER',
        'CREATE_CUSTOMER', 'EDIT_CUSTOMER_INFO', 'ACTIVATE_CUSTOMER',
        'DELETE_CUSTOMER', 'LIST_USERS', 'ASSIGN_USER', 'ASSIGN_SELF']
}
...
def upgrade():
    ...
    try:
        for purpose, actions in LEGACY_ACTIONS.items():
            role_actions = session.query(Action).filter(
                Action.name.in_(actions)
            ).all()
            if len(role_actions) != len(actions):
                raise ValueError('some of actions was not found!')
            role = session.query(Role).filter_by(
                purpose=purpose).one_or_none()
            for role_action in role_actions:
                role.assign_action(role_action)
        session.commit()
    except:
        session.rollback()
        raise
    ...
    try:
        for purpose, actions in LEGACY_ACTIONS.items():
            ...
            role = session.query(Role).filter_by(
                purpose=purpose).one_or_none()
            for role_action in role_actions:
                role.remove_action(role_action)
        session.commit()
    except:
        session.rollback()
        raise
