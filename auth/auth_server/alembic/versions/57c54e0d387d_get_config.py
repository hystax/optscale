...
Revision ID: 57c54e0d387d
Revises: faa30df5ef48
Create Date: 2018-02-22 08:59:41.603132
...
revision = '57c54e0d387d'
down_revision = 'faa30df5ef48'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    try:
        ...
        session.add(glob_mgmt_group)

        root_type = session.query(Type).filter_by(name='root').one_or_none()

        action_get_config = Action(name='GET_CONFIG', type=root_type,
                                   action_group=glob_mgmt_group,
                                   order=1210)
        session.add(action_get_config)
        ...
        try:
            ...
            role_superadmin.assign_action(action_get_config)
...
def downgrade():
    ...
    session = Session(bind=bind)
    try:
        action_get_config = session.query(Action).filter_by(name='GET_CONFIG').one_or_none()
        role_superadmin = session.query(Role).filter_by(
            name='Super Admin').one_or_none()
        role_superadmin.remove_action(action_get_config)
        session.delete(action_get_config)

        session.query(ActionGroup).filter_by(name='GLOBAL_MANAGEMENT').delete()
        session.commit()
    finally:
        session.close()
    # ### end Alembic commands ###
