...
Revision ID: 3bd5237d912e
Revises: 59d180a33635
Create Date: 2020-03-20 10:15:09.118300
...
from sqlalchemy import (
    Column, String, Integer, ForeignKey, TEXT, Boolean, VARCHAR)
...
revision = '3bd5237d912e'
down_revision = '59d180a33635'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    try:
        ...
        session.add(optscale_ag)

        partner_type = session.query(Type).filter_by(
            name='partner').one_or_none()

        action_manage_creds = Action(name='MANAGE_CLOUD_CREDENTIALS',
                                     type=partner_type,
                                     action_group=optscale_ag, order=1310)
        action_manage_resources = Action(name='MANAGE_RESOURCES',
                                         type=partner_type,
                                         action_group=optscale_ag, order=1320)
        action_manage_own_resources = Action(name='MANAGE_OWN_RESOURCES',
                                             type=partner_type,
                                             action_group=optscale_ag,
                                             order=1330)
        action_manage_budgets = Action(name='MANAGE_BUDGETS',
                                       type=partner_type,
                                       action_group=optscale_ag, order=1340)
        action_manage_permissions = Action(name='MANAGE_PERMISSIONS',
                                           type=partner_type,
                                           action_group=optscale_ag,
                                           order=1350)
        session.add(action_manage_creds)
        session.add(action_manage_resources)
        session.add(action_manage_own_resources)
        session.add(action_manage_budgets)
        session.add(action_manage_permissions)

        try:
            role_superadmin = session.query(Role).filter_by(
                name='Super Admin').one_or_none()
            if role_superadmin:
                role_superadmin.assign_action(action_manage_creds)
                role_superadmin.assign_action(action_manage_resources)
                role_superadmin.assign_action(action_manage_own_resources)
                role_superadmin.assign_action(action_manage_budgets)
                role_superadmin.assign_action(action_manage_permissions)
...
def downgrade():
    ...
    session = Session(bind=bind)
    try:
        for name in ['MANAGE_CLOUD_CREDENTIALS', 'MANAGE_RESOURCES',
                     'MANAGE_OWN_RESOURCES', 'MANAGE_BUDGETS',
                     'MANAGE_PERMISSIONS']:
            action = session.query(Action).filter_by(
                name=name).one_or_none()
            role_superadmin = session.query(Role).filter_by(
                name='Super Admin').one_or_none()
            role_superadmin.remove_action(action)
            session.delete(action)
        session.query(ActionGroup).filter_by(
            name='OPTSCALE_MANAGEMENT').delete()
        session.commit()
    finally:
        session.close()
    # ### end Alembic commands ###
