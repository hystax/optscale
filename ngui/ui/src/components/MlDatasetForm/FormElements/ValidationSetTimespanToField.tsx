...
const ValidationSetTimespanToField = ({ name = FIELD_NAMES.VALIDATION_SET_TIMESPAN_TO, isLoading = false }) => {
  const {
      ...
      rules={{
        validate: {
          lessThanOrEqualToTimespanTo: (timespanTo, formValues) => {
            const timespanFrom = formValues[FIELD_NAMES.VALIDATION_SET_TIMESPAN_FROM];
      ...
      render={({ field: { value, onChange } }) => (
        <IntervalTimePicker
          ...
          onApply={(date) => {
            onChange(+date);
            if (isSubmitted) {
              trigger(FIELD_NAMES.VALIDATION_SET_TIMESPAN_FROM);
            }
          }}
          fullWidth
          margin="dense"
...
export default ValidationSetTimespanToField;
