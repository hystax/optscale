...
const Breakdown = ({ runs }) => {
  ...
  const getGoalsBreakdownConfig = () => {
    const goalBreakdownKeyNameMap = Object.fromEntries(
      runs.filter(({ goals }) => !!goals).flatMap((run) => run.goals.map(({ key, name }) => [key, name]))
    );

    return Object.entries(goalBreakdownKeyNameMap).map(([key, name]) => ({
      name: key,
