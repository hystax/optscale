...
const TourUi = ({ steps, close, onStepChange }) => {
  ...
  const { target, content, dataTestId, id } = steps[currentStep] ?? {};
  ...
  // do not how tour before target element bounding rectangle is calculated
  if (!coords.isSet) {
    return null;
  }
