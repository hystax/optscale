...
const MlApplicationParameterFormDefaultGoalValueField = ({ name, isLoading }) => {
  const {
    register,
    formState: { errors }
  } = useFormContext();
  ...
  return isLoading ? (
    ...
    <Input
      name={name}
      label={<FormattedMessage id="defaultGoalValue" />}
      type="number"
      required
      error={!!errors[name]}
...
export default MlApplicationParameterFormDefaultGoalValueField;
