...
const useStyles = makeStyles()((theme, { buttonOpacity, isExpanded }) => ({
  ...
  // padding helps to move away currently selected menu item background, when menu is collapsed
  drawerPaper: {
    position: "relative",
    paddingRight: 0,
    transition: `padding ${ANIMATION_LENGTH / 2}s ${ANIMATION_FUNCTION} 0s`
  },
  drawerPaperHidden: {
    paddingRight: `${INTERACTIVE_BORDER_WIDTH}px`,
    transition: `padding ${ANIMATION_LENGTH}s ${ANIMATION_FUNCTION} ${ANIMATION_LENGTH}s`
  },
  button: {
    position: "absolute",
