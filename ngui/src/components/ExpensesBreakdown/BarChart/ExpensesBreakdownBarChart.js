...
const ExpensesBreakdownBarChart = ({ periodType, pdfId, breakdownData, fieldTooltipText, onClick, filterBy, colorsMap }) => {
  ...
  const { data, keys } = getBarChartDataAndKeys({
    ...
    indexBy: "date",
    sourceData: {
      [EXPENSES_BREAKDOWN_PERIODS.DAILY]: breakdownData.daily,
      [EXPENSES_BREAKDOWN_PERIODS.WEEKLY]: breakdownData.weekly,
      [EXPENSES_BREAKDOWN_PERIODS.MONTHLY]: breakdownData.monthly
    }[periodType]
  });
