...
const LiveDemoContainer = () => {
  ...
  useEffect(() => {
    const activeLiveDemo = (handlers) => (_, getState) => {
      dispatch(getLiveDemo())
        ...
        .then(() => dispatch(createLiveDemo()))
        .then(() => {
          if (isError(CREATE_LIVE_DEMO, getState())) {
            return Promise.reject();
          }

          const email = getState()?.[RESTAPI]?.[GET_LIVE_DEMO].email ?? "";
          const password = getState()?.[RESTAPI]?.[GET_LIVE_DEMO].password ?? "";
          if (email && password) {
  ...
  }, [hasError, dispatch, navigate, authorize]);
