...
from google_calendar_client.client import CalendarException
...
class TestCalendarSynchronizationsApi(TestApiBase):
    def setUp(self, version='v2'):
        ...
        patch('config_client.client.Client.google_calendar_service_key',
              return_value={}).start()
    ...
    def test_delete(self):
        code, c_sync = self.client.calendar_synchronization_create(
            self.valid_calendar_sync_payload)
        self.assertEqual(code, 201)
        with patch('rest_api_server.controllers.calendar_synchronization.'
                   'CalendarSynchronizationController.delete_calendar_events'):
            code, _ = self.client.calendar_synchronization_delete(c_sync['id'])
            self.assertEqual(code, 204)

    @patch('google_calendar_client.client.GoogleCalendarClient.list_events')
    ...
    def test_create_existing_bookings(self):
        ...
        p_create_event = patch(
            'google_calendar_client.client.GoogleCalendarClient.create_event',
            return_value=event
        ).start()
        code, c_sync = self.client.calendar_synchronization_create(
            self.valid_calendar_sync_payload)
