...
elif (mongo "${admin_creds[@]}" "${ssl_args[@]}" --eval "rs.status()" | grep "no replset config has been received"); then
    log "Found existing db with not-initialized replicaset"
    mongo "${admin_creds[@]}" "${ssl_args[@]}" --eval "rs.initiate({'_id': '$replica_set', 'members': [{'_id': 0, 'host': '$service_name'}]})"
