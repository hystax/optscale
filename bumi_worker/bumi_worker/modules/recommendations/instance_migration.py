...
ALIBABA_REGION_MAP = {
    ...
    'cn': ['China (Qingdao)', 'China (Beijing)', 'China (Zhangjiakou)',
           ...
           'China (Guangzhou)', 'China (Chengdu)', 'China (Nanjing)'],
...
class InstanceMigration(ModuleBase):
    ...
    def get_aws_recommendations(self, instance_map, cloud_account_map,
        ...
        for sku_info in res:
            ...
            for sku_dict in similar_skus:
                ...
                for test_region in AWS_REGION_NAMES_TO_CODES.keys():
    ...
    def _get_nearby_alibaba_regions(region):
        for side, regions_list in ALIBABA_REGION_MAP.items():
    ...
    def get_alibaba_recommendations(self, alibaba_instance_map,
        ...
        all_regions = set()
        all_flavors = set()
        for res_id, resource in alibaba_instance_map.items():
        ...
        for region, flavors in prices.items():
            try:
                ...
                LOG.info("Unable to find flavor prices: %s" % str(exc))
                prices[region] = {}
                continue
